...
valA = json.load(open(HERE / "validation/validation_seed5.json"))["sim"]
valB = json.load(open(HERE / "validation/validation_seed9.json"))["sim"]
...
for ax, k in zip(axes, labels):
    ...
    for val, col, mk, lab in ((valA, "C3", "^-", "calibrated, set A (long gap, weak repulsion)"), (valB, "C0", "v--", "calibrated, set B (short gap, strong repulsion)")):
        ...
        ax.errorbar(W, m, yerr=[m - lo, hi - m], fmt=mk, color=col, capsize=3, label=lab)
    for w in W:
...
axes[0].set_title("dotted: held-out widths", fontsize=10); axes[1].legend(fontsize=7, loc="upper right")
