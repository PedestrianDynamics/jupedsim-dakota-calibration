...
set_c = best_parameters("calib_h0_s9/dakota.out")
...
for ax, profile, matrix in zip(axes[:2], profiles, values):
    ...
    ax.plot(best["time_gap"], best["desired_speed"], "o", ms=6, mfc="none",
            mec="white", mew=1.5)
    ax.plot(set_c["time_gap"], 1.55, "x", ms=6, color="white", mew=1.5)
...
axes[2].plot(best["range_neighbor"], best["radius"], "o", ms=6, mfc="none",
             mec="white", mew=1.5)
axes[2].plot(set_c["range_neighbor"], set_c["radius"], "x", ms=6,
             color="white", mew=1.5)
for point in spacing["grid"]:
    bad = sum(point["status_counts"].get(status, 0)
              for status in ("stalled", "failed"))
    if bad == 0:
        continue
    marker, size = ("x", 10) if bad == 9 else (".", 6)
    axes[2].scatter(point["range_neighbor"], point["radius"], marker=marker,
                    s=size, color="black", linewidths=0.6)
